...
def db_save(id_maquina, id_modo, fase1, fase2, fase3, ciclos):
	...
	try:
		...
		insert_tuple = ( formatted_date, id_maquina, id_modo, float(fase1)-Ibase, float(fase2), float(fase3), ciclos )
		#print ('done')
...
def insert_data (id_maquina, id_modo, fase1, fase2, fase3):
	try:
		...
		current_Date = datetime.now()
		formatted_date = current_Date.strftime('%Y-%m-%d %H:%M:%S')
		insert_tuple = (current_Date, id_maquina, id_modo, (float(fase1)-Ibase), (float(fase2)-Ibase), (float(fase3)-Ibase))
		#print ('done')
