...
def MueveImagenes (dias, forigen, fdestino, metodo, porc): #parametros: (delta_days, ruta_origen, ruta_destino, metodo("cp"-copiar, "mv"-mover), porcentaje (se copiaran 1 de cada X imagenes, donde X es porc))
	...
	for arch in lista_archivos:
                ...
                if s_f_arch_a_mover_format == f_arch_format:
                        cont = cont + 1;
				...
				if (metodo=="cp"):
					...
					shutil.move(forigen + arch, fdestino + arch, copy_function=copy2);
...
if (est_ant == 4):
	try:
		MueveImagenes (1, fftp, fusb, "mv", 1);
		write_log ("5");
		est_ant = 5;
	except Exception as inst:
...
if ((est_ant == 5)):
	try:
                MueveImagenes (0, fftp, fweb, "cp", 10); #parametros: (delta_days, ruta_origen, ruta_destino, metodo("cp"-copiar, "mv"-mover), porcentaje)
                write_log ("5");
                est_ant = 5;
        except Exception as inst:
